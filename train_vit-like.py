...
from utils.custom_tokenizer import KoCharElectraTokenizer
...
def main(model_args: ModelArguments, dataset_args: DatasetsArguments, training_args: MyTrainingArguments):
    ...
    if (
        text_model_name == "monologg/kocharelectra-base-discriminator"
        or text_model_name == "monologg/kocharelectra-small-discriminator"
    ):
        print("kocharelectra")
        tokenizer = KoCharElectraTokenizer.from_pretrained(text_model_name)
    else:
    ...
    config.hidden_size = max(vision_config.hidden_size, text_config.hidden_size)  # for deepspeed
